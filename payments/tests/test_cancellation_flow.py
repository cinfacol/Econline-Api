...
class PaymentCancellationFlowTest(TestCase):
    ...
    def test_manual_cancellation_success(self, mock_cancel, mock_retrieve):
        ...
        handle_manual_payment_cancellation_task(
            str(self.payment.id), str(self.user.id), "test_cancellation"
        )
    ...
    def test_session_not_found_in_stripe(self, mock_retrieve):
        ...
        mock_retrieve.side_effect = stripe.error.InvalidRequestError(
            "No such session", "cs_test_session_123"
        )
        ...
        handle_manual_payment_cancellation_task(
            str(self.payment.id), str(self.user.id), "sesión_no_encontrada_en_stripe"
        )
        ...
        with patch("stripe.checkout.Session.retrieve") as mock_retrieve:
            ...
            mock_retrieve.side_effect = [
                ...
                stripe.error.InvalidRequestError(
                    "No such session", "cs_test_session_456"
                ),  # Segunda llamada
            ]
    ...
    def test_cart_coupons_clearing(self):
        ...
        coupon = Coupon.objects.create(
            name="TEST_COUPON", code="TEST10", is_active=True
        )
