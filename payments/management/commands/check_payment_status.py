...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("🔍 Iniciando monitoreo de pagos"))
        ...
        if options["watch"]:
            ...
            self._monitor_with_duration(
                payments, options["interval"], options["duration"]
            )
    ...
    def _monitor_continuously(self, payments, interval):
        ...
        try:
            ...
            while True:
                ...
                self.stdout.write(
                    f"⏳ Esperando {interval} segundos... (Ctrl+C para salir)"
                )
                time.sleep(interval)
                iteration += 1
    ...
    def _monitor_with_duration(self, payments, interval, duration):
        ...
        self.stdout.write(
            f"⏱️  Monitoreando por {duration} segundos ({iterations} iteraciones)"
        )
