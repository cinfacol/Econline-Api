...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('=== PRUEBA DEL CASO REAL: $320 USD, 20%, LÍMITE $35 ===')
        )
        ...
        if total_after_discount == expected_total:
            self.stdout.write(
                self.style.SUCCESS(f"✅ CORRECTO: Total a pagar es ${total_after_discount}")
            )
        else:
            self.stdout.write(
                self.style.ERROR(f"❌ ERROR: Total esperado ${expected_total}, obtenido ${total_after_discount}")
            )
