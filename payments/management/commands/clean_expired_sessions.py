...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Encontrados {pending_payments.count()} pagos pendientes con sesiones de Stripe"
        )
        ...
        for payment in pending_payments:
            try:
                if dry_run:
                    self.stdout.write(
                        f"[DRY RUN] Verificando sesión: {payment.stripe_session_id}"
                    )
                    continue
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(
                    "Ejecutado en modo DRY RUN - No se realizaron cambios"
                )
            )
