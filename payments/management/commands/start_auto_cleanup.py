...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING('Ejecutando en modo DRY RUN - No se iniciará la limpieza automática')
            )
            return
        
        self.stdout.write(
            self.style.SUCCESS(f'Iniciando limpieza automática de sesiones expiradas (delay inicial: {delay}s)')
        )
