...
class Command(BaseCommand):
    ...
    def _find_payments(self, options):
        ...
        if options["user_email"]:
            try:
                ...
                raise CommandError(
                    f"Usuario con email {options['user_email']} no encontrado"
                )
    ...
    def _test_payment_cancellation(self, payment, options):
        ...
        try:
            with transaction.atomic():
                ...
                success = (
                    payment.status == Payment.PaymentStatus.CANCELLED
                    and payment.order.status == Order.OrderStatus.CANCELLED
                )

                if success:
                    self.stdout.write(self.style.SUCCESS("   ✅ Cancelación exitosa"))
                    self.stdout.write(f"      Nuevo estado del pago: {payment.status}")
                    self.stdout.write(
                        f"      Nuevo estado de la orden: {payment.order.status}"
                    )
                else:
                    self.stdout.write(self.style.ERROR("   ❌ Cancelación falló"))
    ...
    def _test_stripe_integration(self, payment):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"   ❌ Error de Stripe: {str(e)}"))
            return False
    ...
    def _show_summary(self, results):
        ...
        if successful == total:
            ...
            self.stdout.write(self.style.WARNING("⚠️  Algunas pruebas fallaron"))
            ...
            for result in results:
                if not result["success"]:
                    self.stdout.write(
                        self.style.ERROR(
                            f"   ❌ Pago {result['payment_id']}: {result.get('error', 'Error desconocido')}"
                        )
                    )
